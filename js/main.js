...
jQuery(document).ready(function ($) {
  ...
  /*
   * ============================================
   * EVALUATION READINESS ONLY
   * Scoped ONLY to #accordion-item-4
   * ============================================
   */

  const $evaluation = $("#accordion-item-4");

  if (!$evaluation.length) {
    return;
  }

  const $progress = $evaluation.find("#evaluation-progress");

  function updateEvaluationProgress() {

    let completed = 0;

    // 1. Normal confirmation checkboxes
    $evaluation
      .find('input.evaluation-checkbox[type="checkbox"]')
      .each(function () {
        if ($(this).is(":checked")) {
          completed++;
        }
    ...
    if (
      $evaluation
        .find('input[name="confidence"]:checked')
        .val() >= 8
    ) {
      completed++;
    }
    ...
    if (
      $evaluation
        .find('input[name="commitment"]:checked')
        .val() >= 8
    ) {
      completed++;
    }
    ...
    // 4. Zoom
    const zoomTotal = $evaluation.find(".zoom-device-check").length;
    const zoomChecked = $evaluation.find(
      ".zoom-device-check:checked"
    ).length;

    if (zoomTotal > 0 && zoomTotal === zoomChecked) {
      completed++;
    }

    $progress.text(completed + " of 10 Complete");
  }
